...
begin
  DM.cdsProduto.Open;
  if Pesquisa(DM.cdsProduto,['DESCRICAO','COD_BARRA','VL_UNIT'],['Nome do Produto','Cód.Barra','Valor'],
...
begin
  ...
  //if Application.MessageBox('Confirma Lançamento?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
  //begin
    if DM.cdsItemPdvLocal.ApplyUpdates(0) <> 0 then
    begin
...
begin
     ...
     with rdprint1 do
       ...
       // impc(05,28,'Fone (14) 1234-5678',[Comp12]);
        ImpF (02,01,'Nº ' +DM.cdsPdvLocalNR_ORCAMENTO.AsString  +
                   '   ' + datetimetostr(now) +
...
//        impval(linha,33,'###,###,##0.00',Total - strtofloat(desconto.text),[]);
        ...
        imp (linha,01,traco);
        inc (linha);
        impc(linha,28,'Obrigado pela preferência',[]);
        inc (linha);
        inc (linha);
...
Var
 Valor : String;
begin
    InputQuery('Reimpressão Orçamento', 'Digite o Nº do Orçamento', Valor);
    if Valor <>'' then
    begin
      ...
      ShowMessage('Nenhum orçamento Selecionado Para Imprimir');
end;
...
begin
  ...
  begin
    ShowMessage('Este Orçamento Foi Cancelado.');
    exit;
  end;
  ...
  //        +' '+' Hora '+DefineTamanhoString(TimeToStr(Time),10,0));
        Memo2.Lines.Add       ('------------------------------------------------------------------------------------');
        Memo2.Lines.Add('DATA DA VENDA...: '+DefineTamanhoString(FormatDateTime('DD/MM/YYYY',cdsReimpressaoDT_ORCAMENTO.AsDateTime),10,0)+'   Hora Impressão..:'+TimeToStr(Time));
