...
begin
  ...
  begin
    ShowMessage('Não é Permitido Reativar Venda Com a Data Anterior a Data Atual.');
    Exit;
  end;
...
procedure TfrmReativarVendas.DBGrid3DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  If cdsCanceladasSTATUS.AsString = 'E'  then // condição
...
procedure TfrmReativarVendas.DBGrid4DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  If cdsItemCanceladaSTATUS.AsString = 'C'  then // condição
