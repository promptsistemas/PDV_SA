...
type
  TfrmDigitaPrecoVenda = class(TForm)
    ...
    procedure DBEdit_DESC_PExit(Sender: TObject);
  private
...
begin
  if Application.MessageBox('Confirma Preço de Venda?','Atenção',MB_YESNO+MB_ICONQUESTION)= idyes  then
  begin
//    DM.cdsItemPdvLocal.ApplyUpdates(0);
  ...
  begin
    ShowMessage('Preço de Venda Não Confirmado.');
    frmPdv.DBEdit2.SetFocus;
    Close;
  end;
end;

procedure TfrmDigitaPrecoVenda.DBEdit_DESC_PExit(Sender: TObject);
begin
  ...
  begin
    ...
    ValorUnit                          :=   (DM.cdsItemPdvLocalVL_UNIT.AsFloat-DM.cdsItemPdvLocalDESC_ITEM.AsFloat);
    DM.cdsItemPdvLocalPERC_DES.AsFloat :=   (ValorUnit/DM.cdsItemPdvLocalVL_UNIT.AsFloat-1)*(100)* -1;
    DM.cdsItemPdvLocalVL_TOTAL.AsFloat :=   (ValorUnit*DM.cdsItemPdvLocalQUANT.AsFloat)-(DM.cdsItemPdvLocalDESC_ITEM.AsFloat*DM.cdsItemPdvLocalQUANT.AsFloat);
  ...
  //  DM.cdsItemPdvLocalVL_UNIT.AsFloat := (ValorUnit-DM.cdsItemPdvLocalDESC_ITEM.AsFloat)
    DM.cdsItemPdvLocalVL_TOTAL.AsFloat :=  (ValorUnit*DM.cdsItemPdvLocalQUANT.AsFloat)-(DM.cdsItemPdvLocalDESC_ITEM.AsFloat*DM.cdsItemPdvLocalQUANT.AsFloat);
  //  DM.cdsItemPdvLocalDESC_ITEM.AsFloat := (ValorUnit-DM.cdsItemPdvLocalVL_UNIT.AsFloat);
...
begin
  ...
  if tp_desconto = 'V' then
  begin
    Label5.Caption := 'Informe o Valor Desconto';
  ...
  begin
    Label5.Caption := 'Informe o % Desconto';
