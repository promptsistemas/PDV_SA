...
begin
  if Application.MessageBox('Deseja Excluir Este Banco ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = idyes then
  begin
    DM.cdsLogin.Delete;
  ...
  begin
    ShowMessage('Usuário Não Excluído');
  end;
    GerenciaBotoes;
end;
...
begin
  Pesquisa(DM.cdsLogin,['LOGIN','NOME_USUARIO'],['Login','Nome do Usuário'],
    [8,25],'','','','','Usuário','Usuário');
end;
...
procedure TfrmCadUsuario.SpeedButton3Click(Sender: TObject);
begin
  Close;
end;
...
begin
  ...
  begin
    ...
    begin
        ...
        begin
          Application.MessageBox('Loja Não Faz Parte do Grupo Informado. Favor Verificar.','Atenção!!!!',MB_OK+MB_ICONWARNING);
          DBEdit4.SetFocus;
        end
        else
        begin
