...
uses
  ...
  DB, DBClient, Provider, SqlExpr, midaslib, Buttons, Menus, RDprint, Printers,
  WinSpool,
...
procedure TfrmPdv.VerificarTroca;
begin
  if DM.cdsPdvLocalNR_ORCAMENTO.AsString <> '' then
  begin
    DM.cdsDinamica.Close;
    DM.cdsDinamica.IndexFieldNames:='';
    DM.cdsDinamica.CommandText :=' SELECT COUNT(IP.QUANT) FROM TB_ITEM_PDV IP WHERE IP.NR_ORCAMENTO = '+DM.cdsPdvLocalNR_ORCAMENTO.AsString+
                                 ' AND IP.QUANT < 0 ';
    DM.cdsDinamica.Open;
...
//  Troca := False;
  ...
  begin
    Application.MessageBox
      ('Nova Venda não Permitida. Finalize a Venda em Andamento' + #13 +
      '           Para Começar uma nova venda.', 'Atenção',
  ...
  begin
    Application.MessageBox
      ('Nova Venda não Permitida. Receba a Venda em Andamento' + #13 +
      '           Para Começar uma nova venda.', 'Atenção',
      MB_OK + MB_ICONWARNING);
    Exit;
  end;
...
//      DM.cdsItemPdvLocalQUANT.AsFloat := 1;
      ...
      DBEdit3.SetFocus; // desabilitar para não informar quantidade;

      if DM.cdsItemPdvLocalVL_UNIT.AsFloat = 0 then
      begin
        ShowMessage('Produto com Preço de Venda Zerado.');
        DBEdit2.SetFocus;
      end
      else
      begin
        // GravaRegistro; //habilitar para não informar quantidade;
      end;
    end
    else
    begin
...
begin
  cdsDinamica.Close;
  cdsDinamica.IndexFieldNames := '';
  cdsDinamica.CommandText := ' SELECT SUM(IP.VL_TOTAL), SUM(IP.DESC_ITEM),SUM(IP.VL_TOTAL_AV), SUM(IP.QUANT)' +
  ...
  L_QT_PECAS.Caption := 'Quant. Peças : '+IntToStr(cdsDinamica.Fields[3].AsInteger);
  DM.cdsPdvLocalVL_LIQUIDO.AsFloat :=
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessage
          ('O Sistema Não Conseguiu Cancelar Esta Venda. Tente Novamente ou Contate o Suporte.');
      end
      else
      begin
        DM.cdsPdvLocal.ApplyUpdates(0);
...
//        DM.cdsPdvLocal.Refresh;
        ...
        L_QT_PECAS.Caption    := 'Quant. Peças';
...
begin
  ...
  if Pesquisa(DM.cdsProduto, ['DESCRICAO', 'COD_BARRA', 'VL_VENDA_PRAZO1'],
    ['Nome do Produto', 'Cód.Barra', 'Vl.Prazo'], [30, 13, 8], '', '', '', '',
...
begin
  ...
  if Pesquisa(DM.cdsProduto, ['COD_BARRA','DESCRICAO', 'VL_VENDA_PRAZO1'],
    ['Cód.Barra','Nome do Produto', 'Valor'], [13,30, 8], '', '', '', '',
    'Produto', 'Produto') then
  begin
...
begin
  ...
  begin
    ...
    // StatusBar1.Panels.Items[3].Text := 'Usuário Logado   '+' '+recInformacoes.login+' ';

  end
  else
  begin
    Application.MessageBox('Usuário sem Loja Definida. Favor Definir uma Loja',
...
begin
  ...
  begin
  //    Application.MessageBox
  //      ('Não é Permitido Realizar Desconto Com a Venda em Aberto.' + #13 +
  //      '   (F4-Para Pausar a Venda Para Realizar o Desconto).', 'Atenção',
  //      MB_OK + MB_ICONWARNING);
  //    Exit;
    L_ENCERRAR_VENDAClick(Self);
  ...
  if L_SITUACAO_VENDA.Caption = 'Caixa Livre' then
  begin
    Application.MessageBox('Não Existe Venda Aberta Para Realizar Desconto.',
      'Atenção', MB_OK + MB_ICONWARNING);
    Exit;
  end;
...
begin
  ...
  begin
    Application.MessageBox
      ('    Impossivél Excluir Iten(s) com a Venda em Andamento' + #13 +
...
//    VerificarTroca;
    ...
    //and (Troca = False) then
    begin
      ShowMessage('Impossivél Receber Esta Venda. O Valor da Venda Esta Zerado.');
      if DM.cdsPdvLocal.RecordCount > 0 then
      begin
        L_SITUACAO_VENDA.Caption := 'LANÇANDO VENDA(S)';
        SituacaoVenda := '';
...
begin
  if L_SITUACAO_VENDA.Caption <> 'Caixa Livre' then
  begin
    Application.MessageBox('Favor Encerre a Venda Para Trocar de Usuário.',
      'Atenção', MB_OK);
  end
  else
  begin
...
begin
  ...
  if (L_SITUACAO_VENDA.Caption = 'LANÇANDO VENDA(S)') and (edtProduto.Text = '') then
  begin
    ShowMessage('Campo Código Produto Não Pode Ser Vazio Com e Venda Em Aberto.');
    edtProduto.SetFocus;
    Exit;
  end;
  if (L_SITUACAO_VENDA.Caption = 'LANÇANDO VENDA(S)') and (edtProduto.Text = '') then
...
begin
  ...
  begin
    ShowMessage
      ('O Sistema Não Conseguiu Salvar Esta Informação. Tente Novamente ou Contate o Suporte.');
  end
  else
  begin
    // grava estoque Movimentacao Geral
...
///
///
    if DM.cdsItemPdvLocalQUANT.AsFloat < 0 then
    begin
      DM.cdsInsertSaidaEstoqueProduto.Params[0].AsInteger :=
        StrToInt(L_LOJA.Caption);
      DM.cdsInsertSaidaEstoqueProduto.Params[1].AsDate :=
        StrToDate(DM.cdsPdvLocalDT_ORCAMENTO.AsString);
      DM.cdsInsertSaidaEstoqueProduto.Params[2].AsString := 'E';
      DM.cdsInsertSaidaEstoqueProduto.Execute;
      DM.cdsEstoqueProduto.ApplyUpdates(0);
      DM.cdsEstoqueProduto.Refresh;
      edtProduto.Clear;
...
begin
  ...
  begin
    ShowMessage
      ('O Sistema Não Conseguiu Gravar Esta Venda. Tente Novamente ou Contate o Suporte.');
  end
  else
  begin
