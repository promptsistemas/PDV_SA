...
uses
  ...
  DBCtrls, QuickRpt, ComCtrls, QRCtrls, ToolWin, FMTBcd, DB,
  DBClient, Provider, SqlExpr, IniFiles, Gauges, Vcl.Imaging.jpeg;
...
procedure TfrmVendedor.SpeedButton3Click(Sender: TObject);
begin
  Close;
end;
...
begin
  if Application.MessageBox(' Deseja Atualizar Cadastro?','Atenção!!!!',MB_YESNO+MB_ICONQUESTION )= idyes then
  begin
    AtualizarCadastro;
  end
  else
...
begin
  ...
  begin
    ...
    cdsVendedorServer.Close;
    cdsVendedorServer.Params[0].AsInteger := StrToInt(frmpdv.recInformacoes.lojaCredenciada);
